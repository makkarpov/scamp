...
class UnsignedShortTypeHandler extends ScalarTypeHandler {
  ...
  override def generateReader(c: blackbox.Context)(src: c.Tree): c.Tree = {
    ...
    q"$src.getShort(_root_.java.nio.ByteOrder.BIG_ENDIAN) & 0xFFFF"
  }
  ...
  override def generateWriter(c: blackbox.Context)(data: c.Tree, dst: c.Tree): c.Tree = {
    ...
    q"$dst.putShort($data.toShort)(_root_.java.nio.ByteOrder.BIG_ENDIAN)"
  }
}
